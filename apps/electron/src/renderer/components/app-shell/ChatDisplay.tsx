...
import { TurnCard, groupMessagesByTurn, formatTurnAsMarkdown, } from "@craft-agent/ui"
...
import type { ChatDisplayProps, ChatDisplayHandle, OverlayState } from "./chat-display-types"
import { isPlanFilePath } from "./chat-display-utils"
import { SettingsActivityCards } from "./SettingsCards"
import { ProcessingIndicator } from "./ProcessingIndicator"
import { MemoizedMessageBubble } from "./MessageBubble"
import { ScrollOnMount } from "./ScrollOnMount"

// Re-export ChatDisplayHandle so existing importers don't break
export type { ChatDisplayHandle } from "./chat-display-types"
...
export const ChatDisplay = React.forwardRef<ChatDisplayHandle, ChatDisplayProps>(function ChatDisplay({
  ...
  const handlePopOut = useCallback((message: { content: string }) => {
    if (!session) return
    setOverlayState({
      type: 'markdown',
  ...
  const handleStructuredResponse = (response: StructuredResponse) => {
    if (response.type === 'permission' && pendingPermission && onRespondToPermission) {
      ...
      const credResponse = response as import("../../../shared/types").CredentialResponse
      onRespondToCredential(
